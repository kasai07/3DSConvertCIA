...
u32 BuildCiaStub(u8* stub, u8* ncchncsd)
{
    ...
    if (memcmp(ncchncsd + 0x100, "NCCH", 4) == 0) {
        ...
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Bad NCCH/NCSD header"); // meaning: developer did not pay attention
        return 0;
    }
    ...
    if (memcmp(cert_hash, cert_hash_expected, 0x20) != 0) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error generating certificate chain");
        return 1;
    }
...
static u32 DecryptCartNcchToFile(u32 offset_cart, u32 offset_file, u32 size, u32 total)
{
    ...
    if ((memcmp(ncch->magic, "NCCH", 4) != 0) || (ncch->size > (size / 0x200))) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error reading partition NCCH header");
        Wait();
		return 1;
    }
    ...
    if (ncch->flags[7] & 0x04) { // for unencrypted partitions...
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Not encrypted, dumping instead...");
        ...
        if (CheckKeySlot(slot_7x, 'X') != 0) {
            DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Slot0x%02XKeyX not set up", slot_7x);
            Wait();
			return 1;
        }
...
u32 DumpCtrGameCart(u32 param)
{
    ...
    if (memcmp(ncch->magic, "NCCH", 4) != 0) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error reading cart NCCH header");
		Wait();
		return 1;
    }
    ...
    if (memcmp(ncsd->magic, "NCSD", 4) != 0) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error reading cart NCSD header");
        Wait();
		return 1;
    }
    ...
    for (u32 i = 0; i < 8; i++) {
        ...
        if (partition->offset < (data_size / 0x200)) {
            DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Overlapping partitions in NCSD table");
            Wait();
    ...
    if (data_size > cart_size) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Used size exceeds cartridge size");
        Wait();
		return 1; // should never happen
    } else if ((data_size < 0x4000) || (cart_size < 0x4000)) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Bad cartridge size");
        Wait();
		return 1; // should never happen
    }
    if (param & CD_MAKECIA) {
        ...
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Cartridge CIA size : %lluMB", dump_size / 0x100000);
        if (dump_size >= 0x100000000) { // should not happen
            DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error: Too big for the FAT32 file system");
            Wait();
			return 1;
        }
    } else {
        dump_size = (param & CD_TRIM) ? data_size : cart_size;
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Cartridge dump size: %lluMB", dump_size / 0x100000);
        if ((dump_size == 0x100000000) && (data_size < dump_size)) {
            ...
            DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Error: Too big for the FAT32 file system");
            if (!(param & CD_TRIM))
                DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"(maybe try dumping trimmed?)");
	...
	if (!FileCreate(filename, true)) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Could not create output file on SD");
        Wait();
		return 1;
    }
    ...
    if (param & CD_MAKECIA) {
        ...
        for (p = 0; p < 3; p++) {
            ...
            next_offset += size;
            if (param & CD_DECRYPT) {
                DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Decrypting partition #%lu (%luMB)...", p, size / 0x100000);
                if (DecryptCartNcchToFile(offset_cart, offset_file, size, dump_size) != 0)
                    break;
            } else {
                DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Dumping partition #%lu (%luMB)...", p, size / 0x100000);
                if (DumpCartToFile(offset_cart, offset_file, size, dump_size, NULL, NULL) != 0)
                    break;
            } 
        }
        if (param & CD_DECRYPT)
            DebugColor((p == 3) ? GREEN : RED, TRANSPARENT, TOP_SCREEN1,"Decryption %s!", (p == 3) ? "success!" : "failed!");
        if (p != 3)
            result = 1;
    } else if (!(param & CD_DECRYPT)) { // dump the encrypted cart
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Dumping cartridge %.16s (%lluMB)...", ncch->productcode, dump_size / 0x100000);
        ...
        if (p == 8) {
            DebugColor(GREEN, TRANSPARENT, TOP_SCREEN1,"Decryption success!");
        } else {
            DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Decryption failed!");
			result = 1;
        }
        
        if ((result == 0) && (dump_size > data_size)) {
            DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Dumping padding (%lluMB)...", (dump_size - data_size) / 0x100000);
    ...
    if ((param & CD_MAKECIA) && (result == 0)) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Finalizing CIA file...");
        if (FixCiaFile(filename) != 0)
            result = 1;
    }
        ...
        if (param & CD_MAKECIA) {
            ...
            for (u32 p = 0; p < 3; p++) {
                ...
                if (size == 0) 
                    continue;
                next_offset += size;
                DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Verifiying partition #%lu (%luMB)...", p, size / 0x100000);
...
u32 DumpNtrGameCart(u32 param)
{
    ...
    if (buff[0] == 0x00) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Error reading cart header");
        return 1;
    }

    memset (name, 0x00, sizeof (name));
    memcpy (name, &buff[0x00], 12);
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Product name: %s", name);

    memset (name, 0x00, sizeof (name));
    memcpy (name, &buff[0x0C], 4 + 2);
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Product ID: %s", name);
    ...
    if (buff[0x12] == 0x02) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Hybrid(DS+DSi) cartridge was detected");
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"This dumper supports only DS mode");
    }
    else if (buff[0x12] != 0x00) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"DSi Cartridge is not supported");
        return 1;
    }

    if (!NTR_Secure_Init (buff, Cart_GetID())) {
        DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Error reading secure data");
        return 1;
	}

    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1," ");
    snprintf(filename, 64, "/Game3ds/nds/%s.nds",name);
...
u32 DumpGameCart(u32 param)
{
    ...
    DebugClear();
	char pathjeu[60];
	snprintf(pathjeu, 60, "Game3ds/menu/MenuBox.tga");
	loadtga(true,false,pathjeu,0,0);
	
    // check if cartridge inserted
    if (REG_CARDCONF2 & 0x1) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"Cartridge was not detected");
        Wait();
		return 1;
    }
    ...
    Cart_Init();
    cartId = Cart_GetID();
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Cartridge ID: %08X", Cart_GetID());
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Cartridge Type: %s", (cartId & 0x10000000) ? "CTR" : "NTR");
    
    // check cartridge type
    if ((cartId & 0x10000000) && (param & CD_NTRCART)) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"NTR cart dump selected but CTR detected");
        Wait();
		return 1;
    } else if (!(cartId & 0x10000000) && !(param & CD_NTRCART)) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN1,"CTR cart dump selected but NTR detected");
        Wait();
		return 1;
    }
...
u32 DumpPrivateHeader(u32 param)
{
    ...
    Cart_Init();
    cartId = Cart_GetID();
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Cartridge ID: %08X", cartId);
    ...
    *(u32*) (privateHeader + 0x4C) = 0xFFFFFFFF;
    ...
    if (!(cartId & 0x10000000)) {
        DebugColor(RED, TRANSPARENT, TOP_SCREEN0,"Error: NTR carts have no private headers");
        Wait();
		return 1;
    }
    ...
    DebugColor(WHITE, TRANSPARENT, TOP_SCREEN1,"Unique ID:");
...
u32 Convert3dstoCIA(u32 index)
{
	
	DebugClear();
	char pathjeu[60];
    
	snprintf(pathjeu, 60, "Game3ds/menu/topbg.tga");
	loadtga(true,false,pathjeu,0,0);
	snprintf(pathjeu, 60, "Game3ds/menu/ciaBox.tga");
	loadtga(true,false,pathjeu,75,45);
	
	
    snprintf(pathjeu, 60, PATH3DS);
	DebugDirOpen(pathjeu);
	u8* buffer = BUFFER_ADDRESS;
    u8* stub = (u8*) 0x20316000;
	...
	while (DirRead(pathjeu, 30)) {
        ...
        u32 stub_size = BuildCiaStub(stub, header);
        if (stub_size == 0) {
            DrawStringFColor(RED, TRANSPARENT, 95, 85, true, "Failed building the CIA stub");
			Wait();
			continue;
        }
        ...
        // write it (first round)
        char filename[256];
		DrawStringFColor(WHITE, TRANSPARENT, 95, 85, true, "Writing CIA stub");
		DrawStringFColor(WHITE, TRANSPARENT, 95, 95, true, "(%lu byte)...", stub_size);
		snprintf(filename, 256, "/%s/%s%s", PATHCIA,c[index],EXTCIA);
        if (FileDumpData(filename, stub, stub_size) != stub_size) {
            DrawStringFColor(RED, TRANSPARENT, 95, 105, true, "Failed writing the CIA stub");
			Wait();
			continue;
        }
        ...
        if (memcmp(header + 0x100, "NCCH", 4) == 0) { // for NCCH files
            ...
            DrawStringFColor(WHITE, TRANSPARENT, 95, 105, true, "Injecting NCCH content (%luMB)...", size / (1024 * 1024));
            ...
            for (p = 0; p < 3; p++) {
				...
				DrawStringFColor(WHITE, TRANSPARENT, 95, 105, true, "Injecting NCSD content");
				if(p == 0)
				{
					DrawStringFColor(WHITE, TRANSPARENT, 95, 115, true, "%lu (%luMB)...", p, size / (1024 * 1024));
				}
				if(p == 1)
				{
					DrawStringFColor(WHITE, TRANSPARENT, 95, 125, true, "%lu (%luMB)...", p, size / (1024 * 1024));
				}
				if(p == 2)
				{
					DrawStringFColor(WHITE, TRANSPARENT, 95, 135, true, "%lu (%luMB)...", p, size / (1024 * 1024));
				}
				if(p == 3)
				{
					DrawStringFColor(WHITE, TRANSPARENT, 95, 145, true, "%lu (%luMB)...", p, size / (1024 * 1024));
				}
				if (FileInjectTo(filename, offset_ncch, offset, size, false, buffer, BUFFER_MAX_SIZE) != size) {
                    
					DrawStringFColor(RED, TRANSPARENT, 95, 155, true, "Content not readable or has bad size");
					Wait();
					break;
                }
            }
        ...
        // Fix the CIA file
        DrawStringFColor(WHITE, TRANSPARENT, 95, 155, true, "Finalizing CIA file...", stub_size);
		
        if (FixCiaFile(filename) != 0) {
            DrawStringFColor(RED, TRANSPARENT, 95, 165, true, "Failed!");
            Wait();
			continue;
        }
		
        DrawStringFColor(GREEN, TRANSPARENT, 95, 165, true, "Success!");
